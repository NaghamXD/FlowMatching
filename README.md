...
Stage 1 covers linear probing and image prototypes only. The codebase is
structured so that Stage 2 (a flow-matching head) and Stage 3 (reusing the
linear-probe setting) can be added without modifying Stage 1 code — see the
module layout below.
...
Stage 1 complete (M0-M8): all 48 runs reproduce from cached features via
`cvlab run-sweep`; results are summarized in `runs/stage1_summary.md` after
running the sweep.
